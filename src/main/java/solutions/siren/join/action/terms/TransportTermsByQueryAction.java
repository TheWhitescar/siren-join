        ...
        extends TransportBroadcastAction<TermsByQueryRequest, TermsByQueryResponse, TermsByQueryShardRequest, TermsByQueryShardResponse>
{
     ...
     */
    @Override
    protected TermsByQueryResponse newResponse(TermsByQueryRequest request, AtomicReferenceArray shardsResponses,
                                               ClusterState clusterState)
    {
        ...
        try {
            ...
            TermsSet termsSet =
                    TermsSet.newTermsSet(expectedElements, request.termsEncoding(), breakerService.getBreaker(CircuitBreaker.REQUEST));
            ...
            try {
                ...
                rsp = new TermsByQueryResponse(termsSet, tookInMillis, shardsResponses.length(), successfulShards, failedShards,
                        shardFailures);
            } finally {
    ...
    protected TermsByQueryShardResponse shardOperation(TermsByQueryShardRequest shardRequest) throws ElasticsearchException, IOException {
        ...
        SearchContext context =
                searchService.createSearchContext(shardSearchRequest, SearchService.NO_TIMEOUT, indexShard.acquireSearcher("termsByQuery"));
            ...
            if (querySource != null && querySource.length() > 0) {
                ...
                try {
                    ...
                    ParsedQuery parsedQuery =
                            orderByOperation.getParsedQuery(queryParser, context.getQueryShardContext());
                    if (parsedQuery != null) {
    ...
    private static abstract class OrderByShardOperation {
         ...
         */
        protected ParsedQuery getParsedQuery(final XContentParser queryParser,
                                             final QueryShardContext queryShardContext) throws IOException
        {
